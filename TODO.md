...
3. Need to think about handling older installers that were Pre Avid Link or maybe put a limit on the oldest versions supported. Avid Link was first bundled around End of 2018/2019?
4. Needs testing with more older .dmg.
5. Save the user's choices to the remove item questions to a text (or JSON?) file and put along side the output .pkg file as a record of what is in it.
...
7. Lots could be done to clean up the script/make it prettier/more modular/data driven but let see if it's of any use first.
