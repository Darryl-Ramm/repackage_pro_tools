...
* The script currently assumes it's working with a recent Pro Tools installer .dmg and offers to uninstall components that may not be in older installer versions. They will be offered as options to remove, but will show as 0 size. Selecting to remove components that are not here will generally not cause problems. It may cause problams for very old installers that pre-date Avid Link being bundled with Pro Tools. This behavior/UX is being improved.
