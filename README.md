...
The script does not currently log the user choices/what was removed, but users can copy and past their interaction with he script into a file and save that as a text file next to the output .pkg. This will likely get addressed in future. 
...
* The script currently assumes it's working with a recent Pro Tools installer .dmg and offers to uninstall components that may not be in older installer versions. Components not actually there will still be offered as options to remove, but will show as 0 size. Selecting to remove components that are not here will generally not cause problems. It may cause problems for very old installers that pre-date Avid Link being bundled with Pro Tools. This behavior/UX is being improved.
